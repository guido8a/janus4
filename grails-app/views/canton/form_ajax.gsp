...
<g:form class="form-horizontal" name="frmSave-cantonInstance" action="save">
    ...
    <div class="form-group ${hasErrors(bean: cantonInstance, field: 'provincia', 'error')} ">
        <span class="grupo">
            ...
            <span class="col-md-6">
                <g:select id="provincia" name="provincia" from="${janus.Provincia.list()}" optionKey="id" optionValue="nombre" disabled="" class="many-to-one form-control" value="${cantonInstance?.provincia?.id ?: janus.Provincia.get(padre)?.id}" noSelection="['null': '']"/>
                <p class="help-block ui-helper-hidden"></p>
            </span>
        </span>
    </div>
    ...
    <div class="form-group ${hasErrors(bean: cantonInstance, field: 'longitud', 'error')} ">
        <span class="grupo">
